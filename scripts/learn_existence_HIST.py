...
TRAIN_IMAGE = toolbox.YES_FULL + r'\yes5\20151181442_0003465.jpg'
COMPARE_IMAGE = toolbox.NO_FULL + r'\6no\20151181442_0007557.jpg'
JSON_FILE = '.\\JsonCenters'
COMBINED_NUM = "combined_num"

TRIAL = 3
COMBINED_JASON = r'.\combined' + str(SEG_NUM) + r'.json'
KP_BF_JASON_WITHOUT = r'.\kp_exist_hist_no_' + str(TRIAL) + r'.json'
KP_BF_JASON_WITH = r'.\kp_exist_hist_yes_' + str(TRIAL) + r'.json'
DIRS = "dir"  # key of the finished directories list

# keys of the histograms in combined
SUPER_NEIGH_HIST = "hist_neighb"
SUPER_HIST = "super_hist"
COMBINED_SUPER_HIST = "combined super hist"
COMBINED_SUPER_NEIGH_HIST = "combined neighbor hist"
...
def preprocess_image(img, seg_nums = SEG_NUM):
...
def find_neighbors(pixel_place, img, super_img):
    ...
    """
    if not in_bounds(pixel_place, img):
        print(pixel_place)
        if pixel_place[0] >= img.shape[0]:
            pixel_place[0] = int(img.shape[0] - 1)
        if pixel_place[1] >= img.shape[1]:
            pixel_place[1] = int(img.shape[1] - 1)
        if pixel_place[0] < 0 or pixel_place[1] < 0:
            print("pixel is minus")
            assert(False)
...
def get_superpixels(ndarray, segments_num = SEG_NUM):
    ...
    # of segments
    segments = slic(image, n_segments=segments_num,
...
#                print("distance" + str(dist))
    ...
    show_region(img_query,find_neighbors(best_pixel, img_query, query_segments), query_segments)

    return best_pixel, best_dist


def get_super_hist(pixel, img, segments, neighbors=True):
    """
    ...
    if neighbors:
        colors = np.asarray([img[neighbor[0], neighbor[1]]
                        for neighbor in find_neighbors(pixel, img, segments)])
    else:
        seg_num = segments[pixel[0], pixel[1]]
        colors = np.asarray([img[pix[0], pix[1]] for pix in
                        get_super_pixels(segments, seg_num)])
    colors = colors.astype(np.float32)
#    images = cv2.cv.fromarray(np.asarray(colors))
    # normalize for a histogram with 1000 pixels
    hist = (1000*calculate_hist(colors))/len(colors)
...
def create_file(dir_path, file_name, to_print=set(toolbox.PRINT_LITTLE), test_hist = None):
    """
...
def create_combined(dir_path, file_name, centers):
    """
    creates a combined picture features and writes it to json and returns it
    Args:
        dir_path - the function searches for pictures recursivly in it
        file_name - the name of the exported file, also reads the file
        for previous info
    ...
    Note:
        ...
        in file_name
    """
    kp = toolbox.import_json(file_name)
    count = 0
    back_count = 0
    super_count = 0
    if COMBINED_NUM not in kp:
        kp[COMBINED_NUM] = 0
    if DIRS not in kp.keys():
        kp[DIRS] = []
    for (cur, dirs, files) in os.walk(dir_path):
        ...
        if cur not in kp[DIRS]:
            for file in files:
                if file not in toolbox.BAD:
                    try:
                        if file not in keys:
                            center = centers[str(file)]
                            center = (center[1], center[0])
                            segments, img = preprocess_image(read_image(cur + "\\" + file))
                            segments_neigh, img_neigh = preprocess_image(read_image(cur + "\\" + file))
                            kp[str(file)] = {}
                            kp[str(file)][SUPER_NEIGH_HIST] = np.ndarray.tolist(get_super_hist(center, img_neigh, segments_neigh, True))
                            kp[str(file)][SUPER_HIST] = np.ndarray.tolist(get_super_hist(center, img, segments, False))
                            count, back_count, super_count = toolbox.save(
                                count, back_count, super_count,
                                kp, file_name, {})
                        if kp[file] is None:
    ...
    counter = 0
    for key in kp:
        if type(kp[key]) != type (0) and SUPER_HIST in kp[key]:
            kp[COMBINED_SUPER_HIST] = np.asarray(kp[key][SUPER_HIST])
            kp[COMBINED_SUPER_NEIGH_HIST] = np.asarray(kp[key][SUPER_NEIGH_HIST])
            break
    ...
    kp[COMBINED_SUPER_HIST] = kp[COMBINED_SUPER_HIST]/counter
    kp[COMBINED_SUPER_NEIGH_HIST] = kp[COMBINED_SUPER_NEIGH_HIST]/counter
    return kp
...
#    raise
    ...
    print("using normalized hists")
    combined = create_combined(DIR_PATH_WITH, COMBINED_JASON, centers)
    print("not using combined")
    combined[COMBINED_SUPER_NEIGH_HIST] = None
    # test compare_images on yes
    kp_yes = create_file(DIR_PATH_WITH, KP_BF_JASON_WITH,
                         to_print=set(toolbox.PRINT_LITTLE),
                         test_hist=combined[COMBINED_SUPER_NEIGH_HIST])
    ...
    kp = create_file(DIR_PATH_WITHOUT, KP_BF_JASON_WITHOUT,
                     to_print=set(toolbox.PRINT_LITTLE),
                     test_hist=combined[COMBINED_SUPER_NEIGH_HIST])
    print_statistics(kp)
    ...
    import pickle
    pickle.dump((y, scores),open("hist_yscores","wb+"))
