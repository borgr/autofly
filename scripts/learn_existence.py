...
EXISTS_THRESHOLD = 0.393
print("need to choose the right threshold")
THRESHOLD = 400
SEG_NUM = 150
...
def exists(img_query):
    img1 = cv2.imread(TRAIN_IMAGE, 0)
    assert(img1 is not None)
    return compare_images(img1, img_query)[1] < EXISTS_THRESHOLD
...
def main():
    ...
    print([thresholds[i] for (i,k) in enumerate(tpr) if k < 0.75 and k > 0.74])

    import pickle
    pickle.dump((y, scores),open("surf_yscores","wb+"))
